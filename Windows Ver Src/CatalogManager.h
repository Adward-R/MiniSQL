...
public: 
	...
	void dropTable(string tablename){
		for(int i = tableNum -1; i >= 0; i--){
			if(tables[i].name == tablename){//删除指定表的信息
	...
	void update(Table& tableinfor){//更新table的信息
		for(int i = 0; i < tableNum; i++){
	...
	void update(Index& index){//更新index的信息
		for(int i = 0; i< indexNum; i++){
	...
	bool ExistTable(string tablename){//判断表是否存在
		int i;
	...
	bool ExistIndex(string tablename, int column){//判断index是否存在
		int i;
	...
	int GetColumnNumber(Table& tableinfo,string columnname)//获取表中某个属性的序号
	{
	...
	int GetColumnAmount(Table& tableinfo){//获取表中属性的数量
