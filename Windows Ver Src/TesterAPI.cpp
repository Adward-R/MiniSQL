...
{	
	...
	case DRPTAB:
		...
		for(int i = 0; i < parsetree.getTableInfo.attriNum; i++){//?????????§Ö?index?????
	...
	case CREIND:
		...
		if(!tableinfor.attributes[indexinfor.column].isPrimeryKey&&!tableinfor.attributes[indexinfor.column].isUnique){//????primary key?????????index
...
int main()
{
	...
	/*		count ++;
			...
			AddSeperator(command);//??command????????????????????¦Â???????????'\0'
		}
